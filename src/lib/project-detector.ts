 ...
 * DEPENDENCIES: Prisma, Anthropic (optional)
 * STATUS: LIVE — uses Claude API when available, falls back to keyword/recency analysis
 */
...
export function getDetectorBackend(): 'claude' | 'local' {
  return hasAnthropicKey() ? 'claude' : 'local';
}
...
export async function detectProjects(
  ...
  if (hasAnthropicKey()) {
    return detectWithClaude(conversations);
  }
...
Respond with this exact JSON structure:
{
  ...
  "keyTopics": ["topic1", "topic2", "topic3"]
    ...
    try {
      ...
      console.error(`[project-detector] Claude API error for "${conv.sessionId}":`, err);
