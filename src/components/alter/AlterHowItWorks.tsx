...
const layers = [
  ...
  },
  {
    ...
    accent: 'from-alter-violet/20 to-transparent',
  },
  {
    icon: Zap,
    name: 'Action engine',
    ...
    accent: 'from-alter-cyan/15 to-transparent',
  },
];

export function AlterHowItWorks() {
  return (
    <section className="alter-section relative border-t border-alter-border/80">
      <div className="pointer-events-none absolute left-1/2 top-0 h-px w-[min(100%,72rem)] -translate-x-1/2 bg-gradient-to-r from-transparent via-alter-primary/25 to-transparent" />
      <div className="alter-container">
        ...
        <div className="mx-auto mt-14 grid max-w-5xl gap-6 lg:grid-cols-3">
          {layers.map((layer) => (
            <div
              key={layer.name}
              className="group relative overflow-hidden rounded-2xl border border-alter-border bg-alter-surface/60 p-6 shadow-alter-card transition hover:border-alter-border-strong md:p-7"
            >
              <div
                className={`pointer-events-none absolute inset-x-0 top-0 h-24 bg-gradient-to-b ${layer.accent} opacity-80`}
              />
              <div className="relative">
                <div className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-xl border border-alter-border-strong bg-alter-bg/80 text-alter-cyan">
