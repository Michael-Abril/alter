  ...
  try {
    const res = await fetch(`${AKASH_ML_API_URL}/chat/completions`, {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${process.env.AKASH_ML_API_KEY}`,
      },
      body: JSON.stringify({
        model: DEFAULT_MODEL,
        messages: [
          { role: 'system', content: 'You are a project analyzer. Return ONLY valid JSON, no markdown, no explanation.' },
    ...
    if (!res.ok) {
      throw new Error(`Akash ML error (${res.status}): ${await res.text()}`);
    }
    ...
    log(`  ⚠️  Akash ML API failed for "${conv.sessionId}": ${err}`);
